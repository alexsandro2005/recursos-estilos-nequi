            ...
            <div class="forms-container">
                <div class="signin-signup">
                    <form action="" method="POST" autocomplete="off" class="sign-in-form">
                        <h2 class="title">Deposito de Dinero</h2>
                        @csrf
                        <div class="input-field">
                            <i class="fas fa-user"></i>
                            <input type="number" min="20000" name="saldo_deposito" required
                                onkeypress="return(multiplenumber(event))"  oninput="maxlengthNumber(this);"
                                minlength="5" maxlength="10" placeholder="Ingrese valor de deposito" />
                        </div>
                        <input type="hidden" value="{{ $user->saldo_inicial }}" name="saldo_inicial" required
                            onkeypress="return(multiplenumber(event))" oninput="maxlengthNumber(this);"
                            placeholder="Ingrese valor de deposito" />
                        <input type="hidden" value="{{$user->id}}" name="documento" required readonly />
                        <input type="hidden" value="{{$user->nombre_completo}}" name="nombre_completo" required readonly />
                        <input type="submit" value="Depositar" class="btn solid" />
                    </form>
                </div>
            </div>
        </div>
